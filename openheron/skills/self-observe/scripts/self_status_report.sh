...
DATA_DIR="${HOME}/.openheron"
LOG_DIR="${DATA_DIR}/log"
TOKEN_DB="${DATA_DIR}/token_usage.db"

echo "=== Openheron Self Status Report ==="
date '+generated_at=%Y-%m-%dT%H:%M:%S%z'

echo
echo "[1] Runtime status"
openheron gateway status --json 2>/dev/null || echo '{"gateway":"unavailable"}'
openheron heartbeat status --json 2>/dev/null || echo '{"heartbeat":"unavailable"}'
openheron provider status --json 2>/dev/null || echo '{"provider":"unavailable"}'
openheron cron status 2>/dev/null || echo 'cron status unavailable'

echo
echo "[2] Token usage"
openheron token stats --json 2>/dev/null || echo '{"token":"unavailable"}'
