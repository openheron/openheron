...
class ProcessSessionManager:
    ...
    def _start_pipe_readers(self, session: ProcessSession) -> None:
        ...
        threading.Thread(
            ...
            daemon=True,
            name=f"openheron-{session.session_id[:8]}-stdout",
        ).start()
        threading.Thread(
            target=_reader,
            args=("stderr", session.process.stderr),
            daemon=True,
            name=f"openheron-{session.session_id[:8]}-stderr",
        ).start()
    ...
    def _start_pty_reader(self, session: ProcessSession) -> None:
        ...
        threading.Thread(
            target=_reader,
            daemon=True,
            name=f"openheron-{session.session_id[:8]}-pty",
        ).start()
    ...
    def _start_exit_watcher(self, session: ProcessSession) -> None:
        ...
        threading.Thread(
            target=_watch,
            daemon=True,
            name=f"openheron-{session.session_id[:8]}-wait",
        ).start()
